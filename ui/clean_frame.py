...
class CleanFrame(ctk.CTkFrame):
    def __init__(self, master, app, **kwargs):
        ...
        self.clean_merge_identical_var = ctk.BooleanVar(value=False)
        ...
        chk_person.pack(anchor="w", pady=(2, 2))

        chk_identical = ctk.CTkCheckBox(config_subframe, text="合并相同字幕行 (1秒内)", variable=self.clean_merge_identical_var, font=self.app.font_normal)
        chk_identical.pack(anchor="w", pady=(2, 0))
    ...
    def task_clean_srt_run(self, paths):
        ...
        if SubtitleTool is None:
            ...
            return
        for msg in SubtitleTool.clean_srt(paths, skip_merge=self.clean_skip_merge_var.get(), clean_uppercase=self.clean_uppercase_var.get(), clean_person=self.clean_person_var.get(), merge_identical=self.clean_merge_identical_var.get()):
            self.app.log(msg)
        self.app.log("任务结束")
